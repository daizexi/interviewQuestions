...
> - 在箭头函数中，this对象引用的是定义箭头函数的作用域，剪头函数不可修改this指向。
...
> 4. 执行构造函数内部的代码。
> 5. 如果构造函数内部返回一个对象，那么返回构造函数中的对象，否则返回内存中的对象。
...
>   - 多态主要由函数重载和函数重写组成。
>     - 函数重载
>       - 函数重载是指子类中定义了和父类相同函数名的函数，但是因为函数签名不一样，所以可以区分。
>     - 函数重写
>       - 函数重写是指子类中定义了和父类中函数名相同且函数签名相同的函数。
...
>
> ```{javascript}
> setTimeout(console.log(1),1000); //这样函数会立即执行
> ```
>
> - 要向setTimeout中的待执行函数传参数，必须使用setTimeout第三个及之后的参数。
>
>   ```{javascript}
...
> 执行上下文保存着自己的作用域链，每个执行上下文都有自己的作用域链，执行上下文在被销毁时，作用域链也会被销毁。（只有活动对象可能因为闭包的存在而被保存）。
